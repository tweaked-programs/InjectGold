package cc.tweaked_programs.injectgold.block.entity;
...
public class InjectorBlockEntity extends BlockEntity implements ImplementedInventory {
    ...
    public InjectorBlockEntity(BlockPos pos, BlockState state) { super(BlockRegister.INJECTOR_BLOCK_ENTITY, pos, state); }
        ...
        if (items.get(0) != ItemStack.EMPTY) {
            if (match.isPresent()) {
                ...
                if (fuel < match.get().getFuelAmount()) {
                    int fuelNeeded = match.get().getFuelAmount() - fuel;
                    bottom = Text.translatable("category."+Main.MOD_ID+".fuel.needed").append(Text.of(Integer.toString(fuelNeeded))).formatted(Formatting.RED);
                }
    ...
    public Direction getFacing() {
        if(this.world.getBlockState(this.pos).getBlock() == BlockRegister.INJECTOR_BLOCK)
